...
def state_gradient_env_dependent(
        system: Union[ParameterizedSystem,ParameterizedSystem2ls],
        initial_state: ndarray,
        target_derivative: Union[Callable, ndarray],
        process_tensors: List[BaseProcessTensor],
        parameters: ndarray,
        mpo_derivatives: List[ndarray],
        start_time: Optional[float] = 0.0,
        num_steps: Optional[int]=None,
        progress_type: Optional[Text] = None) -> Dict:
    """
    ...
    grad_prop, grad_mpo,dynamics = compute_gradient_and_dynamics_env_dependent(
        system=system,
        initial_state=initial_state,
        target_derivative=target_derivative,
        process_tensors=process_tensors,
    ...
    final_derivs = _chain_rule_with_mpo(
        adjoint_tensor_prop=grad_prop,
        adjoint_tensor_mpo=grad_mpo,
        dprop_dparam=get_prop_derivatives,
        propagators=get_half_props,
        num_steps=len(grad_prop),
        num_parameters=num_parameters,
...
def _chain_rule_with_mpo(
        adjoint_tensor_prop:ndarray,
        adjoint_tensor_mpo:ndarray,
        dprop_dparam:Callable[[int], Tuple[ndarray,ndarray]],
        propagators:Callable[[int], Tuple[ndarray,ndarray]],
        num_steps:int,
        num_parameters:int,
    ...
    for i in range(0,num_steps): # populating two elements each step
        ...
        mpo_deriv=combine_derivs(adjoint_tensor_mpo[i], first_half_prop.T, second_half_prop.T)
        for j in range(0,num_parameters):
            total_derivs[2*i][j] = combine_derivs(
                            adjoint_tensor_prop[i],
                            first_half_prop_derivs[j].T,
                            second_half_prop.T) + 0.5*mpo_deriv
                        
            total_derivs[2*i+1][j] = combine_derivs(
                adjoint_tensor_prop[i],
                first_half_prop.T,
                second_half_prop_derivs[j].T) + 0.5*mpo_deriv
...
def compute_gradient_and_dynamics_env_dependent(
        system: Union[ParameterizedSystem,ParameterizedSystem2ls],
        initial_state: ndarray,
        target_derivative: Union[Callable,ndarray],
        process_tensors: List[BaseProcessTensor],
    ...
    # reshape and include caps
    target_ndarray = target_derivative.reshape(hs_dim**2)
    caps = _get_caps(process_tensors, num_steps)
    ...
    combined_deriv_list = []
    combined_deriv_list_mpo = []

    # start from last forward tensor
    backprop_tensor = tn.replicate_nodes([current_node])[0]

    # reversed loop over steps, 0-indexed
    for loop, step in enumerate(reversed(range(num_steps))):

        prog_bar.update(loop)

        # get controls and propagators
        pre_control, post_control = controls(step)
        first_half_prop, second_half_prop = propagators(step)

        # get backward PT MPOs
        pt_mpos_back = _get_pt_mpos_backprop(mpo_list, step)

        # -- apply backprop half-propagators and MPOs --
        current_node, current_edges = _apply_system_superoperator(
            current_node, current_edges, second_half_prop.T)
        current_node, current_edges = _apply_pt_mpos(current_node, current_edges, pt_mpos_back)
        current_node, current_edges = _apply_system_superoperator(
            current_node, current_edges, first_half_prop.T)

        if post_control is not None:
            current_node, current_edges = _apply_system_superoperator(
                current_node, current_edges, post_control.T)
        if pre_control is not None:
            current_node, current_edges = _apply_system_superoperator(
                current_node, current_edges, pre_control.T)

        # forward tensor for this step
        forward_tensor = forwardprop_derivs_list[step]
        forward_tensor_copy = tn.replicate_nodes([forward_tensor])[0]

        # --------------------------
        # derivative w.r.t. props
        # --------------------------
        fwd_edges = forward_tensor[:]
        deriv_forward, fwd_edges = _apply_derivative_pt_mpos(forward_tensor, fwd_edges, mpo_list[step])

        # connect edges with backprop
        for i, _ in enumerate(pt_mpos_back):
            fwd_edges[i] ^ backprop_tensor[i]

        deriv_tensor = deriv_forward @ backprop_tensor
        combined_deriv_list.append(deriv_tensor.get_tensor())

        # --------------------------
        # derivative w.r.t. props and mpo rates
        # --------------------------
        fwd_edges_mpo = forward_tensor_copy[:]
        deriv_forward_mpo, fwd_edges_mpo = _apply_derivative_pt_mpos(forward_tensor_copy, fwd_edges_mpo, mpo_derivatives[step])

        for i, _ in enumerate(pt_mpos_back):
            fwd_edges_mpo[i] ^ backprop_tensor[i]

        deriv_mpo_tensor = deriv_forward_mpo @ backprop_tensor
        combined_deriv_list_mpo.append(deriv_mpo_tensor.get_tensor())

        # update backprop tensor for next iteration
        backprop_tensor = tn.replicate_nodes([current_node])[0]

    # reverse lists to match forward time ordering
    propagator_derivs_out = list(reversed(combined_deriv_list))
    mpo_derivs_out = list(reversed(combined_deriv_list_mpo))

    prog_bar.update(num_steps)
    prog_bar.exit()

    return propagator_derivs_out, mpo_derivs_out, dynamics
